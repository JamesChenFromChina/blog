...
** new 和 delete 
   * new和ｄｅｌｅｔｅ分配和释放对象,　new[]和delete[]用于分配和释放数组
...
** 重载new(放置语法)
   * new运算符可以通过 void* operator new(size_t s, T *p);的方式进行重载，对象的大小的new运算符隐式计算的，所以返回的是void类型指针。该重载为new运算符提供了额外的参数void*p,调用的方式为new（m) X;对象m将会作为p的实参传入到new的重载函数中，m可以是具体的地址或者内存分配器,编译器根据常规的实参匹配规则确定哪个版本的new运算符函数将会被执行，但是每一个operator new函数都以size_t作为第一个实参, 头文件<new>中实现了void*的重载版本，使用void*指针指向的地址存储对象
 //  * 放置式delete 如 void operator delete(void *p, T *allocor) 可能会告知垃圾回收器当前删掉的指针不安全 // shiyan
